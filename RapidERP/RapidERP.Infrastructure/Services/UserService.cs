...
public class UserService(RapidERPDbContext context, ISharedService shared) : IUserService
{
    ...
    public Task<RequestResponse> Create(UserPOST masterPOST)
    {
    ...
    public Task<RequestResponse> Import(List<UserPUT> imports)
    {
        throw new NotImplementedException();
    }
    ...
    public Task<RequestResponse> Update(UserPUT masterPUT)
    {
        throw new NotImplementedException();
    }
    ...
    //        if (skip == 0 || take == 0)
    //        {
    //            result.Count = await shared.GetCounts<User>(pageSize);
