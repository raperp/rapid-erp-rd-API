...
public class CreateBulkActionTypeCommandHandler(IRepository repository)
{
    public async Task<CreateBulkActionTypeCommandResponseModel> Handle(CreateBulkActionTypeCommandRequestModel request)
    {
        CreateBulkActionTypeCommandResponseModel _response = new();
        ...
        try
        {
            foreach (var item in request.masterPOSTs)
            {
                ...
                var isExists = await repository.IsExistsByName<ActionType>(item.Name);
