...
namespace RapidERP.Application.Features.CurrencyFeatures.GetHistoryQuery;

public class GetHistoryCurrencyHandler(IRepository repository)
{
    GetHistoryCurrencyResponseModel _response;

    public async Task<GetHistoryCurrencyResponseModel> Handle(GetHistoryCurrencyRequestModel query)
    {
        try
        {
            GetHistoryDTO result = new();

            var data = (from ca in repository.Set<CurrencyHistory>()
                        join c in repository.Set<Currency>() on ca.CurrencyId equals c.Id
            ...
            //int totalItems = await repository.Set<CurrencyHistory>().CountAsync();
