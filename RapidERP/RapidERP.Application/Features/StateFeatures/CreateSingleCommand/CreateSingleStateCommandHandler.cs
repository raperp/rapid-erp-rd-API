...
namespace RapidERP.Application.Features.StateFeatures.CreateSingleCommand;

public class CreateSingleStateCommandHandler(IRepository repository)
{
    public async Task<CreateSingleStateCommandResponseModel> Handle(CreateSingleStateCommandRequestModel request)
    {
        CreateSingleStateCommandResponseModel _response; 

        try
        {
            using var transaction = repository.BeginTransaction();
            var isExists = await repository.IsExistsByName<State>(request.masterPOST.Name);
