...
public class CreateBulkAreaCommandHandler(IRepository repository)
{
    public async Task<CreateBulkAreaCommandResponseModel> Handle(CreateBulkAreaCommandRequestModel request)
    {
        CreateBulkAreaCommandResponseModel _response = new(); 
        ...
        try
        {
            foreach (var item in request.masterPOSTs)
            {
                ...
                var isExists = await repository.IsExistsByName<Area>(item.Name);
