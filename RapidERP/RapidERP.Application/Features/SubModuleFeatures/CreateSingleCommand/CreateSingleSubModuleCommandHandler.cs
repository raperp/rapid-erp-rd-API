...
public class CreateSingleSubModuleCommandHandler(IRepository repository)
{
    public async Task<CreateSingleSubModuleCommandResponseModel> Handle(CreateSingleSubModuleCommandRequestModel request)
    {
        CreateSingleSubModuleCommandResponseModel _response;

        try
        {
            using var transaction = repository.BeginTransaction();
            var isExists = await repository.IsExistsByName<Submodule>(request.masterPOST.Name);
