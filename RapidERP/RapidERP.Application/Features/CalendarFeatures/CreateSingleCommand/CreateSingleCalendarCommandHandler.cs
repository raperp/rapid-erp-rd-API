...
public class CreateSingleCalendarCommandHandler(IRepository repository)
{
    public async Task<CreateSingleCalendarCommandResponseModel> Handle(CreateSingleCalendarCommandRequestModel request)
    {
        CreateSingleCalendarCommandResponseModel _response; 

        try
        {
            using var transaction = repository.BeginTransaction();
            var isExists = await repository.IsExistsByName<Calendar>(request.masterPOST.Name);
            ...
            if (isExists == false)
            {
                ...
                masterData.EndDate = request.masterPOST.EndDate;
                masterData.TotalMonth = request.masterPOST.TotalMonth;
