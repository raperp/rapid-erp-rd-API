...
public class CreateBulkTextModuleCommandHandler(IRepository repository)
{
    public async Task<CreateBulkTextModuleCommandResponseModel> Handle(CreateBulkTextModuleCommandRequestModel request)
    {
        CreateBulkTextModuleCommandResponseModel _response = new(); 
        ...
        try
        {
            foreach (var item in request.masterPOSTs)
            {
                ...
                var isExists = await repository.IsExistsByName<TextModule>(item.Name);
