...
namespace RapidERP.API.Controllers
{
    ...
    [ApiController]
    public class AreaController(IAreaService area) : ControllerBase
    {
        [HttpGet("GetAll")]
        public async Task<IActionResult> GetAll(int skip, int take, int pageSize)
        {
            var result = await area.GetAll(skip, take, pageSize);
            return Ok(result);
        }
        ...
        {
            var result = await area.GetHistory(skip, take, pageSize);
            return Ok(result);
        }
        ...
        {
            var result = await area.SoftDelete(id);
            return Ok(result);
        }
    }
}
